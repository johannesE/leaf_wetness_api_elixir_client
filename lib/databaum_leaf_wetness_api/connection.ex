...
defmodule DatabaumLeafWetnessAPI.Connection do
  @moduledoc """
  Handle Tesla connections for DatabaumLeafWetnessAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, DatabaumLeafWetnessAPI.Connection,
        base_url: "http://localhost",
        adapter: Tesla.Adapter.Hackney

  The default base URL can also be set as:

      config :databaum_leaf_wetness_api,
        :base_url, "http://localhost"
  """

  @default_base_url Application.compile_env(
                      :databaum_leaf_wetness_api,
                      :base_url,
                      "http://localhost"
                    )
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:databaum_leaf_wetness_api, :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - DatabaumLeafWetnessAPI 0.0.0 - elixir"
        )
      )
